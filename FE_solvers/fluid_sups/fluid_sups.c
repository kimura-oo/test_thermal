...
const int DVAL_OUTPUT_INTERVAL  = 100;
...
// メッシュは各方向51節点50要素 or 101節点100要素 の六面体一次要素限定
void output_result_file_cavity_center_vx(
		VALUES*        vals,
		const char*    directory)
{
	double reynolds = vals->density / vals->viscosity;
	char filename[BUFFER_SIZE];
	snprintf(filename, BUFFER_SIZE, OUTPUT_FILENAME_CAVITY, reynolds);
